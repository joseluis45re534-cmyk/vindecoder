...
// Illustrative example values shown on SAMPLE report pages only — curated static
// content from sample-reports.ts, never a live API call. Mirrors the section
// labels above so the sample page demonstrates exactly what a real report covers.
function sampleSectionRows(sample: SampleReport): { label: string; value: string; icon: typeof FileText }[] {
  const s = sample.sections;
  return [
    { icon: FileText, label: 'Title & brand history', value: s.titleBrand },
    { icon: Car, label: 'Accident history', value: s.accidents },
    { icon: Gauge, label: 'Odometer readings', value: s.odometer },
    { icon: ShieldCheck, label: 'Theft records', value: s.theft },
    { icon: BadgeDollarSign, label: 'Liens & loans', value: s.liens },
    { icon: Gavel, label: 'Auction & sale history', value: s.auction },
    { icon: Bell, label: 'Open recalls', value: s.recalls },
    { icon: Users, label: 'Ownership history', value: s.owners },
  ];
}
...
export default function ReportView({ id, sample }: { id: string; sample?: SampleReport }) {
  ...
  // Sample pages render from curated static data — no fetch, no loading state.
  const initialData: Preview | null = sample
    ? {
        make: sample.make,
        model: sample.model,
        year: sample.year,
  ...
  // pages are the one deliberate exception (clearly labeled illustrative photo).
  const hasRealPhoto = Boolean(data.photoUrl) && !photoFailed;
  const showBrand = !sample && !hasRealPhoto && Boolean(data.brandImageUrl) && !brandFailed;
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-28 lg:pb-12">
      ...
      <div className="bg-gradient-to-br from-slate-950 via-blue-950 to-slate-900 px-4 sm:px-6 lg:px-8 pt-10 pb-24">
        ...
        >
          {sample && !paid ? (
            <span className="inline-flex items-center gap-2 bg-blue-500/15 text-blue-300 text-sm font-semibold px-4 py-1.5 rounded-full ring-1 ring-blue-400/20">
              <FlaskConical className="w-4 h-4" aria-hidden="true" /> Sample report
            </span>
          ) : (
          ...
          <h1 className="font-display text-2xl sm:text-4xl font-bold text-white mt-5 tracking-tight">
            {sample && !paid ? (
              <>
                Sample <span className="text-blue-300">{title}</span> report
              </>
            ) : (
              <>
          ...
          <p className="text-slate-300 mt-3 max-w-xl mx-auto">
            {sample && !paid
              ? 'An illustrative example of what a full CarVinLookup report covers — not a record for a real vehicle.'
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 -mt-16">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-6 items-start">
          ...
          <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-[180px_1fr] gap-6">
                    ...
                    <>
                      ...
                      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-slate-950/80 to-transparent px-3 pt-6 pb-2 flex items-center gap-1.5">
                        ...
                        <span className="text-[10px] text-white font-semibold leading-tight">
                          {sample ? 'Illustrative photo' : 'Photo of this vehicle'}
                        </span>
                      </div>
                    </>
                  ) : showBrand ? (
                ...
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {sample && (
                      <span className="inline-flex items-center gap-1 text-[10px] font-bold text-blue-700 bg-blue-50 px-2 py-0.5 rounded-full uppercase tracking-wide">
                        Sample
                      </span>
                    )}
                  </div>
            ...
            {!paid && sample && (
              /* Sample pages show illustrative example data instead of a locked
                 teaser — this is the point of a sample: real, useful, citable
                 content, clearly not a real vehicle's record. */
              <motion.div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 sm:p-7" {...fadeUp}>
                <div className="flex items-center justify-between mb-5">
                  <h3 className="font-display font-bold text-slate-900">Example report data</h3>
                  <span className="text-[11px] font-bold text-blue-700 bg-blue-50 px-3 py-1 rounded-full uppercase tracking-wider">Illustrative</span>
                </div>
                <motion.div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5" {...gridParent}>
                  {sampleSectionRows(sample).map(({ icon: Icon, label, value }) => (
                    <motion.div key={label} className="flex items-start gap-3 bg-slate-50 rounded-2xl border border-slate-100 p-4" {...gridChild}>
                      <span className="inline-flex w-9 h-9 rounded-lg items-center justify-center text-blue-600 bg-blue-50 shrink-0">
                        <Icon className="w-4 h-4" aria-hidden="true" />
                      </span>
                      <div className="min-w-0">
                        <p className="font-bold text-slate-900 text-sm leading-snug">{label}</p>
                        <p className="text-[12px] text-slate-500 mt-0.5">{value}</p>
                      </div>
                    </motion.div>
                  ))}
                </motion.div>
                <p className="text-xs text-slate-400 mt-5 pt-4 border-t border-slate-100">
                  This is a sample report for illustration only — it does not describe a real vehicle. Run your own VIN to
                  get a report for an actual car.
                </p>
              </motion.div>
            )}
